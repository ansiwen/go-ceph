...
extern int diffIterateCallback(uint64_t ofs, size_t len, int exists, void *index);
...
static inline int wrap_rbd_diff_iterate2(
			...
			uintptr_t arg) {
	return rbd_diff_iterate2(image, fromsnapname, ofs, len, include_parent, whole_object, diffIterateCallback, (void*)arg);
}
*/
import "C"
...
func diffIterateCallback(
	offset C.uint64_t, length C.uint64_t, exists C.int, index unsafe.Pointer) C.int {
